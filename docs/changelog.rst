...
In some cases, jQuery is not loaded with Sphinx 6 on EVAS Docs.
Workaround: You need to add ``sphinx_evas_theme`` to ``extensions`` in your ``conf.py``.
See `readthedocs.org issue #9654`_ for updates.

.. _readthedocs.org issue #9654: https://github.com/ikiwihome/readthedocs.org/pull/9654
...
Many documentation projects depend on ``sphinx-evas-theme`` without specifying a version of the theme (unpinned) while also depending on unpinned versions of Sphinx. The latest version of ``sphinx-evas-theme`` ideally always supports the latest version of Sphinx, but this is now guaranteed.
...
when the theme is used on EVAS Docs.
...
* Add setuptools entry point allowing to use ``sphinx_evas_theme`` as
...
* Yet another patch to deal with extra builders outside Spinx, such as the
  singlehtml builders from the EVAS Docs Sphinx extension
...
.. _#215: https://github.com/rtfd/sphinx_evas_theme/pull/215
